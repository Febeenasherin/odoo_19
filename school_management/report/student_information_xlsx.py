...
class StudentInformation(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write(row, col + 2, 'Last name', headings)
        sheet.write(row, col + 3, 'Department', headings)
        sheet.write(row, col + 4, 'email', headings)
        sheet.write(row, col + 5, 'Phone no', headings)
        ...
        for rec, val in data['grouped_students'].items():
            ...
            sheet.merge_range('D4:D5', rec, txt)
            for stud in val:
                row += 1

                sheet.write(row, col, stud[4], txt)
                sheet.write(row, col + 1, stud[2], txt)
                sheet.write(row, col + 2, stud[3], txt)
                sheet.write(row, col + 3, stud[0], txt)
                sheet.write(row, col + 4, stud[5], txt)
                sheet.write(row, col + 5, stud[6], txt)
